package com.cg.mljrpt.common.security.service;
 ...
 * @date 2021/12/21
 */
public interface MljrptUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new MljrptUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	...
	/**
	 * 通过用户实体查询
	 * @param mljrptUser user
	 * @return
	 */
	default UserDetails loadUserByUser(MljrptUser mljrptUser) {
		return this.loadUserByUsername(mljrptUser.getUsername());
	}
