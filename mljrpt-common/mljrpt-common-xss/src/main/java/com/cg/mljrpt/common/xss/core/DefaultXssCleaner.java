...
package com.cg.mljrpt.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.cg.mljrpt.common.xss.config.MljrptXssProperties;
import com.cg.mljrpt.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final MljrptXssProperties properties;

	public DefaultXssCleaner(MljrptXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(MljrptXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		MljrptXssProperties.Mode mode = properties.getMode();
		if (MljrptXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (MljrptXssProperties.Mode.validate == mode) {
			// 校验
